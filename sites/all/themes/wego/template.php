 ...
 */
function wego_html_head_alter(&$head_elements) {
 ...
 */
function wego_menu_local_tasks(&$variables) {
    $output = '';
 ...
 */
function wego_preprocess_page(&$vars, $hook) {
    if (isset($vars['node'])) {
...
function wego_preprocess_region(&$variables) {
 ...
 */
function wego_process_page(&$variables) {
 ...
 */
function wego_breadcrumb($variables) {
 ...
 */
function wego_preprocess_username(&$vars) {
    global $theme_key;
...
function wego_format_comma_field($field_category, $node, $limit = NULL) {
...
function wego_form_contact_site_form_alter(&$form, $form_state) {
...
function wego_pagination($node, $mode = 'n') {
 ...
 */
function wego_pager_link($variables) {
    $text = $variables['text'];
...
function wego_form_alter(&$form, &$form_state, $form_id) {
    if ($form_id == 'search_block_form') {
        $form['#attributes']['class'][] = 'search_form';
        $form['search_block_form']['#default_value'] = t('Type keywords and press Enter'); // Set a default value for the textfield
        $form['search_block_form']['#attributes']['class'][] = 'form-control';

        // Add extra attributes to the text box
        $form['search_block_form']['#attributes']['onblur'] = "if (this.value == '') {this.value = 'Type keywords and press Enter';}";
        $form['search_block_form']['#attributes']['onfocus'] = "if (this.value == 'Type keywords and press Enter') {this.value = '';}";
        // Prevent user from searching the default text
        $form['#attributes']['onsubmit'] = "if(this.search_block_form.value=='Search'){ alert('Please enter a search'); return false; }";

        // Alternative (HTML5) placeholder attribute instead of using the javascript
        $form['search_block_form']['#attributes']['placeholder'] = t('Type keywords and press Enter');
    }
	
	if(substr($form_id, 0, 7) == 'webform') {
		$form['#attributes']['class'][] = 'block-feedback';
...
/**
 * User CSS function. Separate from wego_preprocess_html so function can be called directly before </head> tag.
 */
function wego_user_css() {
 ...
 */
function wego_preprocess_html(&$vars) {
    global $parent_root;
